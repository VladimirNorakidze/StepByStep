...
from constants import Q, a0, sigma0, T
...
eps  = np.array([0,0.55,0.7,0.9,1.0,1.5,2.0,2.2,2.35,2.5,2.7,
                 ...
                 100,120,150,200,250,300,400,500,600,800,1000,1e17])*(11600./T)
...
sig  = np.array([0,8.39,9.03,9.62,9.83,10.53,17.93,19.5,20.5,21.0,17.5,
                 ...
                 8.2,7.4,6.25,5.6,4.9,4.2,3.5,3.0,2.65,2.15,1.85,1.6,1.25,1.0,0])/(sigma0*1.0e20)
    
"""Функция сечения упругого взаимодействия:"""
elastic = interpolate.UnivariateSpline(eps, sig, s=0)

def r_excitation(energy, J0, treshold):
    sigma = 0.
    if (energy > treshold): #Если меньше, то сечение равно нулю
        sigma = (8./15)*np.pi*(Q*a0)**2*np.sqrt(1-treshold/energy)*( ( (J0+1)*(J0+2) )/( (2*J0+1)*(2*J0+3) ) )
    return sigma/sigma0

def r_deexcitation(energy, J0, treshold):
    sigma = 0.
    if energy > 0.:
        sigma = (8./15)*np.pi*(Q*a0)**2*np.sqrt(1+treshold/energy)*( ( (J0+1)*(J0+2) )/( (2*J0+5)*(2*J0+3) ) )
    return sigma/sigma0
